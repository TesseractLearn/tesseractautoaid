  ...
  const fetchNearbyMechanics = useCallback(async (lat: number, lng: number, radiusKm = 150) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('id, user_id, full_name, specialization, latitude, longitude, rating, is_available, is_verified, address, services_offered, profile_photo_url, experience_years, total_jobs_count, total_rating_count, active_jobs_count, recent_jobs_count, avg_response_time, avg_eta, created_at, updated_at');
